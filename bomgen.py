...
# modified 2018-07-07, sort reference designators within each part and then sort whole BOM by part type (A, B, C ...)
# modified 2022-03-01, convert to Python3, still expecting Kicad XSL-created BOMs.
#                      Remove total cost, it doesn't work after sort.
#                      Eliminate some fields that we don't care about.
#
...
# The first is the csv BOM as exported from Kicad. 
# The second is the csv of the parts database.
# The third is the resulting BOM, also CSV
...
for bomEntry in bomList:
    ...
    PartNum = bomEntry[' PN']
    print('\t\t', RefDes, Value, Footprint, PartNum)
...
for thisPart in lookupList:
    ...
    # Now match company part number to orderable part number
    for part in partsList:
        if part['Part Number'] == thisPart['PartNum']:
            ...
            finalBomList.append({'Part Number': thisPart['PartNum'], 'count': thisPart['count'], 'Vendor P/N': part['Vendor P/N'], 'Value': thisPart['value'], 'Vendor': part['Vendor'],'RefDesList': thisPart['RefDesList'], 'ext. cost': cost, 'Qty on hand': part['Quantity On Hand']})
            break;
    else:
        # part was not found!
        finalBomList.append({'Part Number': thisPart['PartNum'], 'count': thisPart['count'], 'Vendor P/N': '????', 'Value': thisPart['value'], 'Vendor': '????', 'RefDesList': thisPart['RefDesList'], 'ext. cost': 0})
...
finalBomFile = sys.argv[3] #'FinalBom.csv'
fieldNames = ['Part Number', 'count', 'Vendor P/N', 'Value', 'Vendor', 'RefDesList', 'ext. cost', 'Qty on hand']
...
with open(finalBomFile, 'w') as f:
    ...
    writer.writeheader()
    writer.writerows(finalBomList)
f.close()
print ('DONE! BomFile ', sys.argv[3], 'written.')
